"""Train RandomForest model for 2-class NMS (neutral vs eyebrow_raise)."""
...
def run(config_path: str) -> None:
    ...
    metadata_path = Path(dataset_cfg["metadata_dir"]) / "nms_frames_train.csv"
    ...
    if df.empty:
        raise RuntimeError("nms_frames_train.csv is empty.")
    ...
    try:
        for idx, row in df.iterrows():
            ...
            if (idx + 1) % 200 == 0:
                print(f"Processed {idx + 1}/{len(df)} nms frames")
    finally:
        extractor.close()

    if not features:
        raise RuntimeError("No usable features extracted from nms frames.")
    ...
    save_confusion_matrix(
        ...
        title="NMS Confusion Matrix (Holdout)",
    )
    ...
    holdout_manifest.to_csv("artifacts/nms_holdout_manifest.csv", index=False)

    merge_metrics_file("reports/metrics.json", "nms_train", metrics)

    print(f"Saved model: {model_path}")
    print(f"Saved encoder: {encoder_path}")
    print("Saved report: reports/nms_confusion_matrix.png")
